...
func generateKeyPair(makeSyncable: Bool) -> Data {
    ...
    if !seed.withUnsafeMutableBytes({ 0 == ed25519_create_seed($0) }) {
        print("\nERROR: Unable to initialize random seed")
        exit(1)
    }

    seed.withUnsafeBytes { seed in
        publicEdKey.withUnsafeMutableBytes { publicEdKey in
            privateEdKey.withUnsafeMutableBytes { privateEdKey in
...
print("""
    This tool uses the macOS Keychain to store a private key for signing app updates
    which will be distributed via Sparkle. The key will be associated with your user
    account. If you have iCloud Keychain enabled, the key may optionally be marked as
...
if let pubKey = findPublicKey() {
    ...
    print("""
        A new signing key will be generated.
    ...
    let pubKey = generateKeyPair(makeSyncable: makeSyncable!)
