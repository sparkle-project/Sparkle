...
{
    if (!encodedSignature || !pkeyString || !path)
        return NO;
    BOOL result = NO;
    ...
    if (!pubKey)
        return NO;
    if ((cspHandle = cdsaInit()) == CSSM_INVALID_HANDLE)
        goto validate_end; // Init CDSA
    if (!cdsaVerifyKey(cspHandle, pubKey))
        goto validate_end; // Verify the key is valid
    if ((pathData = [NSData dataWithContentsOfFile:path]) == nil)
        goto validate_end; // File data
    if ((hashData = cdsaCreateSHA1Digest(cspHandle, (CFDataRef)pathData)) == NULL)
        goto validate_end; // Hash
    ...
    if ((sigData = b64decode(encodedSignature)) == nil)
        goto validate_end; // Decode signature
...
static NSData *b64decode(NSString *str)
{
    ...
    if ((a = calloc(4, sizeof(UInt8))) == NULL)
        return nil;
    ...
    do
    {
        ...
        if (ai >= 2)
            buf[j + 0] = (UInt8)((a[0] << 2) | (a[1] >> 4));
        if (ai >= 3)
            buf[j + 1] = (UInt8)((a[1] << 4) | (a[2] >> 2));
        if (ai >= 4)
            buf[j + 2] = (UInt8)((a[2] << 6) | (a[3] >> 0));
        j += ai - 1;
    } while (i < len);
...
static void cdsaRelease(CSSM_CSP_HANDLE cspHandle)
{
    if (CSSM_ModuleDetach(cspHandle) != CSSM_OK)
        return;
...
static CSSM_KEY_PTR cdsaCreateKey(CFDataRef rawKey)
{
    ...
    if (!rawKey || (CFDataGetLength(rawKey) == 0))
        return NULL;

    if ((retval = su_malloc(sizeof(CSSM_KEY), NULL)) == NULL)
        return NULL;
...
static CSSM_DATA_PTR su_createData(CFDataRef bytes)
{
    ...
    data->Length = 0;
    if (bytes)
        su_copyBytesToData(data, (CSSM_SIZE)CFDataGetLength(bytes), CFDataGetBytePtr(bytes));
    return data;
}
...
static void su_freeData(CSSM_DATA_PTR data, Boolean freeData)
{
    if (data)
    {
        if (freeData && data->Data)
            su_free(data->Data, NULL);
        su_free(data, NULL);
    }
}
